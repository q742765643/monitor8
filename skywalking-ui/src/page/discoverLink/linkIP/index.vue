<template>
  <div class="linkIPTemplate">
    ...
    <div id="tablediv" class="tableDateBox">
      <div class="ipNameClass">全网IP列表</div>
      <!-- <a-row type="flex" class="rowToolbar" :gutter="10">
        <a-col :span="1.5">
          <a-button type="primary" icon="plus" @click="handleAdd"> 新增 </a-button>
        </a-col>
      ...
      <vxe-table :data="hostLists" align="center" highlight-hover-row border ref="hostVxe">
        ...
        <vxe-table-column field="currentStatus" title="在线状态" show-overflow>
          <template v-slot="{ row }">
            <span v-if="row.currentStatus == 2"> 不在线</span>
            <span v-else> 在线</span>
          </template>
        </vxe-table-column>
      ...
      <vxe-pager
        id="page_table"
        perfect
        :current-page.sync="IPParams.pageNum"
        :page-size.sync="IPParams.pageSize"
        :total="ipPaginationTotal"
        :page-sizes="[10, 20, 100]"
        :layouts="['PrevJump', 'PrevPage', 'Number', 'NextPage', 'NextJump', 'Sizes', 'FullJump', 'Total']"
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item label="ip地址" prop="ip">
              <a-input v-model="IPFormDialog.ip" placeholder="请输入ip地址"> </a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="在线状态" prop="currentStatus">
              <a-input v-model="IPFormDialog.currentStatus" placeholder="请输入在线状态"> </a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="地址" prop="address">
              <a-input v-model="IPFormDialog.address" placeholder="请输入地址"> </a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
            <a-form-model-item :label-col="{ span: 3 }" :wrapperCol="{ span: 21 }" label="备注" prop="jobDesc">
              <a-input type="textarea" v-model="IPFormDialog.jobDesc" placeholder="请输入备注内容"> </a-input>
            </a-form-model-item>
          </a-col>
        </a-row>
      </a-form-model>
