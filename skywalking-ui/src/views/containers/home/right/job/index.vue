      ...
      <a-form-model layout="inline" :model="queryParams" ref="queryForm">
        ...
        <a-form-model-item label="执行状态" prop="triggerStatus">
          <a-select style="width: 120px" v-model="queryParams.triggerStatus" placeholder="字典状态">
            <a-select-option v-for="dict in statusOptions" :value="dict.dictValue">
              {{ dict.dictLabel }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
      ...
      <a-form-model :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" :model="form" ref="form">
        ...
        <a-form-model-item label="调度执行器" prop="jobHandler">
          <a-select v-model="form.jobHandler" placeholder="调度执行器">
            <a-select-option v-for="dict in jobHandlerOptions" :value="dict.dictValue">
              {{ dict.dictLabel }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
        <a-form-model-item label="状态" prop="triggerStatus">
          <a-radio-group v-model="form.triggerStatus">
            <a-radio v-for="dict in statusOptions" :value="dict.dictValue">{{ dict.dictLabel }}</a-radio>
          </a-radio-group>
        </a-form-model-item>
