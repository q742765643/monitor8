...
<script lang="js">
  ...
  import zoom from '../../../components/topology/chart/utils/zoom';
  import { simulationInit, simulationSkip } from '../../../components/topology/chart/utils/simulation';
  import nodeElement from '../../../components/topology/chart/utils/nodeElement';
  import { linkElement, anchorElement } from '../../../components/topology/chart/utils/linkElement';
  import tool from '../../../components/topology/chart/utils/tool';
  export default {
    props: {
      current: {
        type: Object,
    ...
    mounted() {
      ...
      this.svg.on('click', (d, i) => {
        ...
        this.$store.commit('rocketTopoNetwork/SET_NODE', {});
        this.$store.commit('rocketTopoNetwork/SET_LINK', {});
        this.$store.dispatch('rocketTopoNetwork/CLEAR_TOPO_INFO');
    ...
    methods: {
      ...
      handleNodeClick(d) {
       ...
       /* this.$emit('setCurrent', { key: d.id, label: d.name });
        ...
        this.$store.dispatch('rocketTopoNetwork/CLEAR_TOPO_INFO');
        this.$store.commit('rocketTopoNetwork/SET_NODE', rest);
        this.$store.commit('rocketTopoNetwork/SET_LINK', {});*/
        event.stopPropagation();
        this.$store.commit('rocketTopoNetwork/SET_NODE', {});
        this.$store.commit('rocketTopoNetwork/SET_LINK', d);
        this.$store.dispatch('rocketTopoNetwork/CLEAR_TOPO_INFO');
        this.$store.commit('rocketTopoNetwork/SET_MODE', d.detectPoints);
        this.$store.dispatch(this.$store.state.rocketTopo.mode ? 'rocketTopoNetwork/GET_TOPO_SERVICE_INFO' :
                'rocketTopoNetwork/GET_TOPO_CLIENT_INFO', { ...d, duration: this.$store.getters.durationTime });
        this.$store.commit('rocketTopoNetwork/SET_CALLBACK', () => {
          this.$store.dispatch(this.$store.state.rocketTopo.mode ? 'rocketTopoNetwork/GET_TOPO_SERVICE_INFO' :
                  'rocketTopoNetwork/GET_TOPO_CLIENT_INFO', { ...d, duration: this.$store.getters.durationTime });
        });
      },
