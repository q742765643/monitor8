package com.piesat.constant;

public class IndexNameConstant {
    public static final String METRICBEAT = "metricbeat";
    public static final String T_MT_ALARM_LOG = "t_mt_alarm_log";
    public static final String T_MT_ALARM = "t_mt_alarm";
    public static final String T_MT_FILE_MONITOR = "t_mt_file_monitor";
    public static final String T_MT_FILE_STATISTICS = "t_mt_file_statistics";
    public static final String T_MT_MEDIA_REPORT = "t_mt_media_report";
    public static final String T_MT_MEDIA_OVERVIEW = "t_mt_media_overview";
    public static final String T_MT_HOST_DOWN_LOG = "t_mt_host_down_log";
    public static final String T_MT_PROCESS_DOWN_LOG = "t_mt_process_down_log";
    public static final String T_MT_PROCESS_REPORT="t_mt_process_report";

}
