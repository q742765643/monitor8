...
/**
 * Implementation of the SchemaMetaDataExtractor contract which uses the standard JDBC {@link DatabaseMetaData}
 * API for extraction.
 *
...
public class InformationExtractorJdbcDatabaseMetaDataImpl implements InformationExtractor {
    ...
    private void populateTablesWithColumns(
        ...
        Map<String, TableInformation> map = tables.getTables();
        map.forEach((key, value) -> {
            try {
				/*String[] ttt=(String.valueOf(value.getName().getTableName())).split("|.");
				String catalogFilterXugu=ttt[0];
				String  schemaFilterXugu=ttt[1];*/
                String tableNamePattern = key;
